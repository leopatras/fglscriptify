...
FUNCTION setPathFor(arr,envName,cmd)
  DEFINE arr DYNAMIC ARRAY OF STRING
  DEFINE envName STRING
  DEFINE cmd STRING
  DEFINE i INT
  IF arr.getLength()>0 THEN
    LET cmd=cmd,envName,"="
    LET cmd=IIF(m_bat,"set ",""),cmd
    ...
    FOR i=1 TO arr.getLength()
        ...
        END IF
        LET cmd=cmd,os.Path.join(tmpdir,arr[i])
    END FOR
    LET cmd=cmd,IIF(m_bat,"&&"," ")
  END IF
  RETURN cmd
...
FUNCTION runLastModule() --we must get argument quoting right
  ...
  IF cmd!=cmdsave AND os.Path.exists(image2font) THEN
    IF m_bat THEN
      LET cmd=cmd,";",image2font,"&&"
    ELSE
      LET cmd=cmd.subString(1,cmd.getLength()-1),":",image2font," "
    END IF
  END IF
